...
export const Indent = Extension.create({
  ...
  addKeyboardShortcuts() {
    ...
    const isInListItem = (state: { selection: { $from: { depth: number; node: (d: number) => { type: { name: string } } } } }) => {
      const { $from } = state.selection;
      for (let d = $from.depth; d > 0; d--) {
    ...
    return {
      Tab: () => {
        ...
        if (isInListItem(state as never)) return false;
      ...
      "Shift-Tab": () => {
        ...
        if (inCode) {
          ...
          else if (lineHead.startsWith("    ")) removeLen = 4;
          else if (lineHead.startsWith("  ")) removeLen = 2;
          else if (lineHead.startsWith(" ")) removeLen = 1;
        } else {
          // 일반 문단: 줄 머리의 NBSP 4개 또는 1개 제거
          if (lineHead.startsWith(INDENT_NBSP)) removeLen = 4;
          else if (lineHead.startsWith("\u00a0")) removeLen = 1;
        }
        if (!removeLen) return true; // Tab 포커스 이탈 방지
